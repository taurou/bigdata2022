...
public class SparkDriver {
	...
	public static void main(String[] args) {
		...
		SparkSession ss = SparkSession.builder()
				.master("local").appName("Spark Exercise #48 - SQL").getOrCreate();
		...
		Dataset<NameAgeProfile> dsProfilesMaleIncreasedAgeSorted = 
				ss.sql("SELECT name, age+1 as age FROM "
						+ "profiles WHERE gender='male' "
						+ "ORDER BY age desc, name asc")
				.as(Encoders.bean(NameAgeProfile.class));
