...
class MapperBigData2 extends Mapper<LongWritable, // Input key type
	...
	protected void setup(Context context) throws IOException, InterruptedException {
		...
		BufferedReader fileFriends = 
				new BufferedReader(new FileReader(new File(new Path(CachedFiles[0].getPath()).getName())));
		
		/* We can also simply use the following line of code instead of the previous one since we know 
		  apriori the name of the shared file
